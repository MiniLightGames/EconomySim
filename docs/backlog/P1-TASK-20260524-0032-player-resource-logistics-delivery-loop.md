...
- `BuyResourceCommand` supports `pickup` and `delivery` modes.
- Delivery creates a `Shipment` and keeps purchased input unavailable until logistics delivery.
- `ResourcePurchase` now links to shipment and records goods/logistics cost split.
- Shipment delivery updates linked purchase status and only then adds buyer inventory.
- API and UI expose delivery state, and production is disabled while input shipments are in transit.
