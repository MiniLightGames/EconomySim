...
type MutableInfrastructureLink = Mutable<InfrastructureLink>;
type MutableTransportCompany = Mutable<TransportCompany>;
type MutableBank = Mutable<Bank>;
type MutableCentralBank = Mutable<CentralBank>;
type MutableBankAccount = Mutable<BankAccount>;
type MutableLoan = Mutable<Loan>;
type MutableCreditScore = Mutable<CreditScore>;
type MutableOrder = Mutable<Order>;
type MutableOrderBook = Omit<Mutable<OrderBook>, "bids" | "asks"> & {
  bids: MutableOrder[];
  asks: MutableOrder[];
};
type MutablePortfolioPosition = Mutable<PortfolioPosition>;
type MutableBankruptcyCase = Mutable<BankruptcyCase>;
type MutableAssetAuction = Mutable<AssetAuction>;
type MutableGovernment = Mutable<Government>;
type MutablePoliticalParty = Omit<Mutable<PoliticalParty>, "policyBias"> & { policyBias: LawType[] };
type MutableElection = Omit<Mutable<Election>, "results"> & { results: Array<Mutable<Election["results"][number]>> };
type MutableLaw = Mutable<Law>;
type MutableTaxPolicy = Mutable<TaxPolicy>;
type MutableGovernmentBudget = Mutable<GovernmentBudget>;
type MutablePublicDebt = Omit<Mutable<PublicDebt>, "bondIds"> & { bondIds: string[] };
type MutableSubsidy = Mutable<Subsidy>;
type MutableLicense = Mutable<License>;
type MutableSanctionPolicy = Mutable<SanctionPolicy>;
type MutableCorruptionIndex = Mutable<CorruptionIndex>;
type MutableProtest = Mutable<Protest>;
type MutableLobbyingAction = Mutable<LobbyingAction>;
type MutableMediaInfluence = Mutable<MediaInfluence>;
type MutableWar = Mutable<War>;
type MutableFront = Omit<Mutable<Front>, "cellIds"> & { cellIds: string[] };
type MutableStrategicCell = Mutable<StrategicCell>;
type MutableArmy = Mutable<Army>;
type MutableMilitaryUnit = Omit<Mutable<MilitaryUnit>, "supplyNeedPerTick"> & { supplyNeedPerTick: Mutable<MilitaryUnit["supplyNeedPerTick"]> };
type MutableMilitarySupply = Mutable<MilitarySupply>;
type MutableOccupation = Mutable<Occupation>;
type MutableTreaty = Omit<Mutable<Treaty>, "countryIds"> & { countryIds: string[] };
type MutableSanction = Mutable<Sanction>;
type MutableAlliance = Omit<Mutable<Alliance>, "countryIds"> & { countryIds: string[] };
type MutableBlockade = Mutable<Blockade>;
type MutableRefugeeFlow = Mutable<RefugeeFlow>;
type MutableWarDamage = Mutable<WarDamage>;
type MutableMilitaryOrder = Mutable<MilitaryOrder>;
type MutableTechnology = Omit<Mutable<Technology>, "effects" | "prerequisites"> & {
  effects: Mutable<Technology["effects"]>;
  prerequisites: string[];
};
type MutableTechnologyLevel = Mutable<TechnologyLevel>;
type MutableResearchProject = Mutable<ResearchProject>;
type MutablePatent = Mutable<Patent>;
type MutableLicenseAgreement = Mutable<LicenseAgreement>;
type MutablePollution = Mutable<Pollution>;
type MutableEnvironmentalIndex = Mutable<EnvironmentalIndex>;
type MutableResourceDeposit = Mutable<ResourceDeposit>;
type MutableResourceDiscovery = Mutable<ResourceDiscovery>;
type MutableCleanEnergyPolicy = Mutable<CleanEnergyPolicy>;
type MutableBlackMarket = Mutable<BlackMarket>;
type MutableIllegalTrade = Mutable<IllegalTrade>;
type MutableSmugglingRoute = Mutable<SmugglingRoute>;
type MutableCorruptionCase = Mutable<CorruptionCase>;
type MutableInvestigation = Mutable<Investigation>;
type MutableEnforcementAgency = Mutable<EnforcementAgency>;
type MutableFine = Mutable<Fine>;
type MutableConfiscation = Mutable<Confiscation>;
type MutableReputationPenalty = Mutable<ReputationPenalty>;
type MutableIllegalContract = Mutable<IllegalContract>;
type MutableEventCause = Mutable<EventCause>;
type MutableEventImpact = Mutable<EventImpact>;
type MutableMetricChange = Mutable<MetricChange>;
type MutableExplanation = Omit<Mutable<Explanation>, "causes" | "impactIds" | "relatedMetricIds" | "relatedEntityIds"> & {
  causes: Array<Mutable<Explanation["causes"][number]>>;
  impactIds: string[];
  relatedMetricIds: string[];
  relatedEntityIds: string[];
};
type MutableRetailOffer = Mutable<RetailOffer>;
type MutableNewsTemplate = Mutable<NewsTemplate>;
type MutableForecast = Omit<Mutable<Forecast>, "driverExplanationIds"> & { driverExplanationIds: string[] };
type MutablePublicStatistic = Mutable<PublicStatistic>;
type MutableHiddenStatistic = Mutable<HiddenStatistic>;
...
const NEED_CATEGORIES: readonly NeedCategory[] = ["food", "housing", "transport", "medicine", "entertainment"];

const NEED_PROFILE: Record<NeedCategory, { readonly unitsPerPersonPerTick: number; readonly budgetShare: number }> = {
...
export function runTick(input: TickInput): TickResult {
  ...
  const inventoryLots: MutableInventoryLot[] = loadedState.inventoryLots.map((lot) => ({ ...lot }));
  const retailOffers: MutableRetailOffer[] = loadedState.retailOffers.map((offer) => ({ ...offer }));
  const retailPriceChanges: RetailPriceChange[] = [...loadedState.retailPriceChanges];
  const shipments: MutableShipment[] = loadedState.shipments.map((shipment) => ({ ...shipment }));
  const logisticsRoutes: MutableLogisticsRoute[] = loadedState.logisticsRoutes.map((route) => ({
    ...route,
    nodeIds: [...route.nodeIds],
    infrastructureLinkIds: [...route.infrastructureLinkIds]
  }));
  const infrastructureLinks: MutableInfrastructureLink[] = loadedState.infrastructureLinks.map((link) => ({ ...link }));
  const transportCompanies: MutableTransportCompany[] = loadedState.transportCompanies.map((company) => ({ ...company }));
  const centralBanks: MutableCentralBank[] = loadedState.centralBanks.map((bank) => ({ ...bank }));
  const banks: MutableBank[] = loadedState.banks.map((bank) => ({ ...bank }));
  const bankAccounts: MutableBankAccount[] = loadedState.bankAccounts.map((account) => ({ ...account }));
  const loans: MutableLoan[] = loadedState.loans.map((loan) => ({ ...loan }));
  const creditScores: MutableCreditScore[] = loadedState.creditScores.map((score) => ({ ...score }));
  const orderBooks: MutableOrderBook[] = loadedState.orderBooks.map((book) => ({
    ...book,
    bids: book.bids.map((order) => ({ ...order })),
    asks: book.asks.map((order) => ({ ...order }))
  }));
  const portfolioPositions: MutablePortfolioPosition[] = loadedState.portfolioPositions.map((position) => ({ ...position }));
  const bankruptcies: MutableBankruptcyCase[] = loadedState.bankruptcies.map((bankruptcy) => ({ ...bankruptcy }));
  const assetAuctions: MutableAssetAuction[] = loadedState.assetAuctions.map((auction) => ({ ...auction }));
  const governments: MutableGovernment[] = loadedState.governments.map((government) => ({ ...government }));
  const politicalParties: MutablePoliticalParty[] = loadedState.politicalParties.map((party) => ({
    ...party,
    policyBias: [...party.policyBias]
  }));
  const elections: MutableElection[] = loadedState.elections.map((election) => ({
    ...election,
    results: election.results.map((result) => ({ ...result }))
  }));
  const laws: MutableLaw[] = loadedState.laws.map((law) => ({ ...law, parameters: { ...law.parameters } }));
  const taxPolicies: MutableTaxPolicy[] = loadedState.taxPolicies.map((policy) => ({ ...policy }));
  const governmentBudgets: MutableGovernmentBudget[] = loadedState.governmentBudgets.map((budget) => ({ ...budget }));
  const publicDebt: MutablePublicDebt[] = loadedState.publicDebt.map((debt) => ({ ...debt, bondIds: [...debt.bondIds] }));
  const subsidies: MutableSubsidy[] = loadedState.subsidies.map((subsidy) => ({ ...subsidy }));
  const licenses: MutableLicense[] = loadedState.licenses.map((license) => ({ ...license }));
  const sanctionPolicies: MutableSanctionPolicy[] = loadedState.sanctionPolicies.map((policy) => ({ ...policy }));
  const corruptionIndexes: MutableCorruptionIndex[] = loadedState.corruptionIndexes.map((index) => ({ ...index }));
  const protests: MutableProtest[] = loadedState.protests.map((protest) => ({ ...protest }));
  const lobbyingActions: MutableLobbyingAction[] = loadedState.lobbyingActions.map((action) => ({ ...action }));
  const mediaInfluences: MutableMediaInfluence[] = loadedState.mediaInfluences.map((influence) => ({ ...influence }));
  const wars: MutableWar[] = loadedState.wars.map((war) => ({ ...war }));
  const fronts: MutableFront[] = loadedState.fronts.map((front) => ({ ...front, cellIds: [...front.cellIds] }));
  const strategicCells: MutableStrategicCell[] = loadedState.strategicCells.map((cell) => ({ ...cell, center: { ...cell.center } }));
  const armies: MutableArmy[] = loadedState.armies.map((army) => ({ ...army }));
  const militaryUnits: MutableMilitaryUnit[] = loadedState.militaryUnits.map((unit) => ({
    ...unit,
    supplyNeedPerTick: { ...unit.supplyNeedPerTick }
  }));
  const militarySupplies: MutableMilitarySupply[] = loadedState.militarySupplies.map((supply) => ({ ...supply }));
  const occupations: MutableOccupation[] = loadedState.occupations.map((occupation) => ({ ...occupation }));
  const treaties: MutableTreaty[] = loadedState.treaties.map((treaty) => ({ ...treaty, countryIds: [...treaty.countryIds] }));
  const sanctions: MutableSanction[] = loadedState.sanctions.map((sanction) => ({ ...sanction }));
  const alliances: MutableAlliance[] = loadedState.alliances.map((alliance) => ({ ...alliance, countryIds: [...alliance.countryIds] }));
  const blockades: MutableBlockade[] = loadedState.blockades.map((blockade) => ({ ...blockade }));
  const refugeeFlows: MutableRefugeeFlow[] = loadedState.refugeeFlows.map((flow) => ({ ...flow }));
  const warDamage: MutableWarDamage[] = loadedState.warDamage.map((damage) => ({ ...damage }));
  const militaryOrders: MutableMilitaryOrder[] = loadedState.militaryOrders.map((order) => ({ ...order }));
  const technologies: MutableTechnology[] = loadedState.technologies.map((technology) => ({
    ...technology,
    effects: { ...technology.effects },
    prerequisites: [...technology.prerequisites]
  }));
  const technologyLevels: MutableTechnologyLevel[] = loadedState.technologyLevels.map((level) => ({ ...level }));
  const researchProjects: MutableResearchProject[] = loadedState.researchProjects.map((project) => ({ ...project }));
  const patents: MutablePatent[] = loadedState.patents.map((patent) => ({ ...patent }));
  const licenseAgreements: MutableLicenseAgreement[] = loadedState.licenseAgreements.map((agreement) => ({ ...agreement }));
  const pollution: MutablePollution[] = loadedState.pollution.map((record) => ({ ...record }));
  const environmentalIndexes: MutableEnvironmentalIndex[] = loadedState.environmentalIndexes.map((index) => ({ ...index }));
  const resourceDeposits: MutableResourceDeposit[] = loadedState.resourceDeposits.map((deposit) => ({ ...deposit }));
  const resourceDiscoveries: MutableResourceDiscovery[] = loadedState.resourceDiscoveries.map((discovery) => ({ ...discovery }));
  const cleanEnergyPolicies: MutableCleanEnergyPolicy[] = loadedState.cleanEnergyPolicies.map((policy) => ({ ...policy }));
  const blackMarkets: MutableBlackMarket[] = loadedState.blackMarkets.map((market) => ({ ...market }));
  const illegalTrades: MutableIllegalTrade[] = loadedState.illegalTrades.map((trade) => ({ ...trade }));
  const smugglingRoutes: MutableSmugglingRoute[] = loadedState.smugglingRoutes.map((route) => ({ ...route }));
  const corruptionCases: MutableCorruptionCase[] = loadedState.corruptionCases.map((item) => ({ ...item }));
  const investigations: MutableInvestigation[] = loadedState.investigations.map((item) => ({ ...item }));
  const enforcementAgencies: MutableEnforcementAgency[] = loadedState.enforcementAgencies.map((agency) => ({ ...agency }));
  const fines: MutableFine[] = loadedState.fines.map((fine) => ({ ...fine }));
  const confiscations: MutableConfiscation[] = loadedState.confiscations.map((confiscation) => ({ ...confiscation }));
  const reputationPenalties: MutableReputationPenalty[] = loadedState.reputationPenalties.map((penalty) => ({ ...penalty }));
  const illegalContracts: MutableIllegalContract[] = loadedState.illegalContracts.map((contract) => ({ ...contract }));
  const eventCauses: MutableEventCause[] = loadedState.eventCauses.map((cause) => ({ ...cause }));
  const eventImpacts: MutableEventImpact[] = loadedState.eventImpacts.map((impact) => ({ ...impact }));
  const metricChanges: MutableMetricChange[] = loadedState.metricChanges.map((change) => ({ ...change }));
  const explanations: MutableExplanation[] = loadedState.explanations.map((explanation) => ({
    ...explanation,
  ...
  const newsTemplates: MutableNewsTemplate[] = loadedState.newsTemplates.map((template) => ({ ...template }));
  const forecasts: MutableForecast[] = loadedState.forecasts.map((forecast) => ({
    ...forecast,
    driverExplanationIds: [...forecast.driverExplanationIds]
  }));
  const publicStatistics: MutablePublicStatistic[] = loadedState.publicStatistics.map((statistic) => ({ ...statistic }));
  const hiddenStatistics: MutableHiddenStatistic[] = loadedState.hiddenStatistics.map((statistic) => ({ ...statistic }));
  const trades: Trade[] = [...loadedState.trades];
  const economyEvents: DomainEvent[] = [];
  ...
  const newsEvents = news.map<DomainEvent>((item) => ({
...
export function runTicks(state: WorldState, count: number, seed = "scenario"): WorldState {
...
export function buyResource(state: WorldState, input: BuyResourceInput, seed = "operations"): BuyResourceResult {
...
export function runManualProduction(
  state: WorldState,
  input: RunManualProductionInput,
  seed = "operations"
): RunManualProductionResult {
  const loadedState = normalizeWorldState(state);
...
export function setRetailOfferPrice(
  state: WorldState,
  input: SetRetailOfferPriceInput,
  seed = "operations"
): SetRetailOfferPriceResult {
  const loadedState = normalizeWorldState(state);
  const retailOffers: MutableRetailOffer[] = loadedState.retailOffers.map((offer) => ({ ...offer }));
  const retailPriceChanges: RetailPriceChange[] = [...loadedState.retailPriceChanges];
...
export function startResearchProject(
  state: WorldState,
  input: StartResearchProjectInput,
  seed = "technology"
): StartResearchProjectResult {
  const loadedState = normalizeWorldState(state);
  ...
  const existingActiveProject = loadedState.researchProjects.some(
    (project) => project.companyId === company.id && project.technologyId === technology.id && project.status === "active"
  );
...
export function createShipment(state: WorldState, input: CreateShipmentInput, seed = "logistics"): CreateShipmentResult {
  const loadedState = normalizeWorldState(state);
  const quantity = sanitizeQuantity(input.quantity);
...
export function quoteShipment(state: WorldState, route: LogisticsRoute, quantity: number, transportCompanyId?: string): ShipmentQuote {
  const loadedState = normalizeWorldState(state);
  ...
  const borderBlocked = loadedState.borderCrossings.find(
    (border) => route.nodeIds.includes(border.nodeId) && (!border.open || border.sanctionLevel >= 0.85)
  );
  ...
  const risk = clamp(
    average(links.map((link) => link.warDisruptionRisk + (1 - link.quality) * 0.25)) +
...
export function applyForLoan(state: WorldState, input: LoanApplicationInput, seed = "finance"): LoanApplicationResult {
  const loadedState = normalizeWorldState(state);
  ...
  const countryRate =
    loadedState.interestRates.find((rate) => rate.countryId === lender.countryId)?.primeRate ??
...
export function payLoan(state: WorldState, input: LoanPaymentInput, seed = "finance"): LoanPaymentResult {
  const loadedState = normalizeWorldState(state);
...
export function placeOrder(state: WorldState, input: CreateOrderInput, seed = "finance"): CreateOrderResult {
  const loadedState = normalizeWorldState(state);
  const quantity = sanitizeQuantity(input.quantity);
  ...
  const createdOrder =
    book.bids.find((candidate) => candidate.id === order.id) ?? book.asks.find((candidate) => candidate.id === order.id) ?? order;
...
export function createIllegalTrade(state: WorldState, input: CreateIllegalTradeInput, seed = "crime"): CreateIllegalTradeResult {
  const loadedState = normalizeWorldState(state);
  const quantity = sanitizeQuantity(input.quantity);
  ...
  const route = input.smugglingRouteId
    ...
    : loadedState.smugglingRoutes.find(
        (candidate) =>
          candidate.active &&
          !candidate.blocked &&
          candidate.destinationCityId === blackMarket.cityId &&
          (candidate.productId === null || candidate.productId === product.id)
      ) ?? null;
  ...
  const buyerAccount = getSettlementAccount(
    loadedState.bankAccounts.map((account) => ({ ...account })),
    input.buyerOwnerType,
    input.buyerOwnerId,
    seller.currencyCode
  );
...
export function fundLobbying(state: WorldState, input: LobbyingInput, seed = "government"): LobbyingResult {
  const loadedState = normalizeWorldState(state);
  const amountMinor = sanitizeMoney(input.amountMinor);
...
export function runMediaCampaign(state: WorldState, input: MediaCampaignInput, seed = "government"): MediaCampaignResult {
  const loadedState = normalizeWorldState(state);
  ...
  const mediaInfluence: MediaInfluence = {
...
export function castVote(state: WorldState, input: VoteInput, seed = "government"): VoteResult {
  const loadedState = normalizeWorldState(state);
  const country = loadedState.countries.find((candidate) => candidate.id === input.countryId);
  ...
  const politicalParties: MutablePoliticalParty[] = loadedState.politicalParties.map((candidate) => ({
    ...candidate,
  ...
  const winner = election.results.reduce<(typeof election.results)[number] | null>(
    (leader, candidate) => (!leader || candidate.totalVotes > leader.totalVotes ? candidate : leader),
    null
  );
...
function selectRoute(state: WorldState, input: CreateShipmentInput): LogisticsRoute | null {
  if (input.routeId) {
  ...
  return (
    state.logisticsRoutes.find(
      (route) =>
        route.originWarehouseId === input.originWarehouseId &&
        route.destinationWarehouseId === input.destinationWarehouseId &&
        (!input.transportCompanyId || route.transportCompanyId === input.transportCompanyId)
    ) ?? null
  );
}
...
function getRouteHandlingCostMinor(state: WorldState, route: LogisticsRoute, quantity: number): number {
...
function calculateLoanCapacityMinor(state: WorldState, bank: Bank, score: CreditScore): number {
...
function getOrCreateCreditScore(
  scores: MutableCreditScore[],
  borrowerType: CreditScore["borrowerType"],
  borrowerId: string,
  tick: number,
  seed: string
): MutableCreditScore {
  ...
  const score: MutableCreditScore = {
...
function ensureBankAccount(input: {
  readonly accounts: MutableBankAccount[];
  readonly bankId: string;
  readonly ownerType: BankAccount["ownerType"];
  readonly ownerId: string;
  readonly currencyCode: BankAccount["currencyCode"];
  readonly seed: string;
}): MutableBankAccount {
  const existing = input.accounts.find(
    (account) =>
  ...
  const account: MutableBankAccount = {
...
function getBorrowerAccount(accounts: readonly MutableBankAccount[], loan: Loan): MutableBankAccount | null {
  return (
    accounts.find(
      (account) =>
        account.ownerType === loan.borrowerType &&
...
function applyLoanPaymentMutable(input: {
  readonly loan: MutableLoan;
  readonly amountMinor: number;
  readonly banks: MutableBank[];
...
}): { readonly paidPrincipalMinor: number; readonly paidInterestMinor: number } {
...
function markLoanDefaulted(context: FinanceContext, loan: MutableLoan, reason: string): void {
...
function isExchangeAssetTradeable(state: WorldState, assetType: FinancialAssetType, assetId: string): boolean {
  if (assetType === "stock") {
...
function getOrCreateOrderBook(
  orderBooks: MutableOrderBook[],
  exchange: Exchange,
...
function getSettlementAccount(
  accounts: readonly MutableBankAccount[],
  ownerType: BankAccount["ownerType"],
  ownerId: string,
  currencyCode: BankAccount["currencyCode"]
): MutableBankAccount | null {
  return (
    accounts.find(
      (account) =>
...
function getAvailableCashMinor(account: BankAccount): number {
  return sanitizeMoney(account.balanceMinor - account.reservedMinor);
}

function debitPlayerAccount(
  accounts: MutableBankAccount[],
  playerId: string,
  currencyCode: BankAccount["currencyCode"],
  amountMinor: number
): MutableBankAccount {
...
function playerHasPoliticalEligibility(state: WorldState, playerId: string): boolean {
  return (
    state.bankAccounts.some((account) => account.ownerType === "player" && account.ownerId === playerId && getAvailableCashMinor(account) > 0) ||
...
function requiresIndustryLicense(state: WorldState, company: Company, industry: Product["category"]): boolean {
  return state.laws.some(
    (law) =>
      law.countryId === company.countryId &&
...
function hasActiveIndustryLicense(state: WorldState, company: Company, industry: Product["category"]): boolean {
  return state.licenses.some(
    (license) =>
      license.countryId === company.countryId &&
...
function getLatestBudget(budgets: readonly MutableGovernmentBudget[], countryId: string): MutableGovernmentBudget | null {
  return budgets
...
function countOrders(orderBooks: readonly OrderBook[]): number {
  return orderBooks.reduce((total, book) => total + book.bids.length + book.asks.length, 0);
}

function matchOrderBook(input: {
  readonly book: MutableOrderBook;
  readonly bankAccounts: MutableBankAccount[];
  readonly portfolioPositions: MutablePortfolioPosition[];
...
function transferPosition(input: {
  readonly positions: MutablePortfolioPosition[];
  ...
  const seller = input.positions.find(
    (position) =>
      position.ownerType === input.sellerOwnerType &&
  ...
  const buyer = input.positions.find(
    (position) =>
      position.ownerType === input.buyerOwnerType &&
...
interface ApplyAcceptedPlayerCommandInput {
  readonly state: WorldState;
  ...
  readonly productionPlans: MutableProductionPlan[];
...
function applyAcceptedPlayerCommand(input: ApplyAcceptedPlayerCommandInput): void {
  const { command } = input;
...
function applyCreateCompanyCommand(input: ApplyAcceptedPlayerCommandInput, command: Extract<PlayerCommand, { type: "CreateCompanyCommand" }>): void {
  ...
  const nameExists = input.companies.some(
    (company) => company.countryId === command.countryId && company.name.toLocaleLowerCase() === normalizedName.toLocaleLowerCase()
  );
...
function applyBuyLandCommand(input: ApplyAcceptedPlayerCommandInput, command: Extract<PlayerCommand, { type: "BuyLandCommand" }>): void {
  ...
  const starterProductionPlan: MutableProductionPlan | null =
    wheatProduct && breadProduct
      ...
      : null;
  const starterRetailOffer: MutableRetailOffer | null =
    breadProduct && starterProductionPlan
      ...
      : null;
  const foodLicenseLaw = input.state.laws.find(
    (law) => law.countryId === company.countryId && law.type === "industry_license" && law.status === "active" && law.parameters.industry === "food"
  );
...
function applyRunManualProductionCommand(
  input: ApplyAcceptedPlayerCommandInput,
  command: Extract<PlayerCommand, { type: "RunManualProductionCommand" }>
): { readonly productionRun: ManualProductionRun } {
...
interface ApplyRetailPriceUpdateInput {
  readonly state: WorldState;
  readonly retailOffers: MutableRetailOffer[];
  readonly retailPriceChanges: RetailPriceChange[];
  readonly events: DomainEvent[];
  readonly metrics: Metric[];
  readonly news: NewsItem[];
  readonly input: Omit<SetRetailOfferPriceInput, "retailOfferId"> & {
    readonly retailOfferId?: string;
    readonly productId?: string;
    readonly commandId?: string;
  };
...
function applyRetailPriceUpdate(input: ApplyRetailPriceUpdateInput): {
  readonly retailOffer: RetailOffer;
  readonly priceChange: RetailPriceChange;
} {
  ...
  const priceChange: RetailPriceChange = {
...
function validateCommand(state: WorldState, command: PlayerCommand): RejectedCommand | null {
  if (command.type === "CreateCompanyCommand") {
...
function addGameHours(isoDate: string, hours: number): string {
...
function processProduction(context: ProductionContext): void {
...
function getAdjustedInputQuantityPerOutput(quantityPerOutput: number, inputEfficiency: number): number {
...
function getTechnologyEffectForCompany(
  context: Pick<ProductionContext, "technologies" | "technologyLevels">,
  company: Company,
  product: WorldState["products"][number],
  effectKey: keyof Technology["effects"]
): number {
  ...
  return clamp(
    ...
    0,
    effectKey === "inputEfficiency" || effectKey === "pollutionReduction" ? 0.55 : 1
  );
}

function technologyMatchesProduct(technology: Technology, product: WorldState["products"][number]): boolean {
  return (
...
interface TechnologyEcologyContext {
  readonly state: WorldState;
  readonly companies: MutableCompany[];
  readonly cities: MutableCity[];
  readonly populationCohorts: MutablePopulationCohort[];
  readonly bankAccounts: MutableBankAccount[];
  readonly technologies: MutableTechnology[];
  readonly technologyLevels: MutableTechnologyLevel[];
  readonly researchProjects: MutableResearchProject[];
  readonly patents: MutablePatent[];
  readonly licenseAgreements: MutableLicenseAgreement[];
  readonly pollution: MutablePollution[];
  readonly environmentalIndexes: MutableEnvironmentalIndex[];
  readonly resourceDeposits: MutableResourceDeposit[];
  readonly resourceDiscoveries: MutableResourceDiscovery[];
  readonly cleanEnergyPolicies: MutableCleanEnergyPolicy[];
  readonly nextTick: number;
  readonly seed: string;
  readonly financialTransactions: FinancialTransaction[];
  readonly events: DomainEvent[];
...
function processTechnologyAndEcology(context: TechnologyEcologyContext): void {
...
function processResearchProjects(context: TechnologyEcologyContext): void {
  for (const project of context.researchProjects) {
    ...
    if (company) {
      ...
      const companyAccount = context.bankAccounts.find(
        (account) => account.ownerType === "company" && account.ownerId === company.id && account.status === "active"
      );
...
function applyTechnologyLicenses(context: TechnologyEcologyContext): void {
  for (const agreement of context.licenseAgreements) {
    ...
    const existing = context.technologyLevels.find(
      (level) => level.technologyId === agreement.technologyId && level.scopeType === agreement.scopeType && level.scopeId === agreement.scopeId
    );
...
function processResourceDeposits(context: TechnologyEcologyContext): void {
...
function processIndustrialPollution(context: TechnologyEcologyContext): void {
  for (const metric of context.metrics) {
    ...
    const pollutionRecord: Pollution = {
...
function applyEnvironmentalPressure(context: TechnologyEcologyContext): void {
...
function updateEnvironmentalIndex(
  context: TechnologyEcologyContext,
  city: City,
  amount: number,
...
function getPollutionFactor(product: WorldState["products"][number]): number {
...
function getCountryTechnologyEffect(
  context: Pick<TechnologyEcologyContext, "technologies" | "technologyLevels">,
  countryId: string,
  effectKey: keyof Technology["effects"]
): number {
  return clamp(
    context.technologyLevels
    ...
    0,
    effectKey === "pollutionReduction" ? 0.7 : 1
  );
}

function upsertTechnologyLevel(input: {
  readonly levels: MutableTechnologyLevel[];
  readonly seed: string;
  readonly nextTick: number;
  readonly technologyId: string;
  readonly scopeType: TechnologyLevel["scopeType"];
  readonly scopeId: string;
  readonly level: number;
  readonly progress: number;
}): void {
  const existing = input.levels.find(
    (level) => level.technologyId === input.technologyId && level.scopeType === input.scopeType && level.scopeId === input.scopeId
  );
...
interface DemandContext {
  ...
  readonly nextTick: number;
  readonly seed: string;
  readonly demandRecords: DemandRecord[];
  readonly financialTransactions: FinancialTransaction[];
  readonly events: DomainEvent[];
  readonly metrics: Metric[];
}

function processPopulationDemand(context: DemandContext): void {
...
interface BuyForNeedInput {
  readonly state: WorldState;
  readonly companies: MutableCompany[];
  readonly cohort: MutablePopulationCohort;
  readonly inventoryLots: MutableInventoryLot[];
  readonly cityId: string;
  readonly needCategory: NeedCategory;
  readonly desiredQuantity: number;
  readonly maxCategorySpendMinor: number;
  readonly nextTick: number;
  readonly seed: string;
  readonly financialTransactions: FinancialTransaction[];
  readonly events: DomainEvent[];
...
function buyForNeed(input: BuyForNeedInput): PurchaseResult {
...
function processLogistics(context: LogisticsContext): void {
...
interface FinanceContext {
  readonly state: WorldState;
  readonly centralBanks: MutableCentralBank[];
  readonly banks: MutableBank[];
  readonly bankAccounts: MutableBankAccount[];
  readonly companies: MutableCompany[];
  readonly loans: MutableLoan[];
  readonly creditScores: MutableCreditScore[];
  readonly orderBooks: MutableOrderBook[];
  readonly portfolioPositions: MutablePortfolioPosition[];
  readonly bankruptcies: MutableBankruptcyCase[];
  readonly assetAuctions: MutableAssetAuction[];
  readonly trades: Trade[];
  readonly nextTick: number;
  readonly seed: string;
  readonly financialTransactions: FinancialTransaction[];
...
function processFinance(context: FinanceContext): void {
...
function processCentralBankBondPurchases(context: FinanceContext): void {
  for (const centralBank of context.centralBanks) {
    const eligibleBond = context.state.bonds.find(
      (bond) => bond.currencyCode === centralBank.currencyCode && bond.centralBankEligible && !bond.defaulted
    );
    ...
    const reserveAccount = context.bankAccounts.find(
      (account) => account.ownerType === "bank" && account.ownerId === commercialBank.id && account.accountType === "reserve"
    );
...
function processLoanInterestAndPayments(context: FinanceContext): void {
...
function processCreditRisk(context: FinanceContext): void {
...
function processCompanyBankruptcies(context: FinanceContext): void {
  for (const loan of context.loans) {
    ...
    const bankruptcy: MutableBankruptcyCase = {
    ...
    const auction: MutableAssetAuction = {
...
function processBankBankruptcies(context: FinanceContext): void {
...
function processAssetAuctions(context: FinanceContext): void {
...
interface GovernmentContext {
  readonly state: WorldState;
  readonly countries: MutableCountry[];
  readonly governments: MutableGovernment[];
  readonly politicalParties: MutablePoliticalParty[];
  readonly elections: MutableElection[];
  readonly laws: MutableLaw[];
  readonly taxPolicies: MutableTaxPolicy[];
  readonly governmentBudgets: MutableGovernmentBudget[];
  readonly publicDebt: MutablePublicDebt[];
  readonly subsidies: MutableSubsidy[];
  readonly licenses: MutableLicense[];
  readonly sanctionPolicies: MutableSanctionPolicy[];
  readonly corruptionIndexes: MutableCorruptionIndex[];
  readonly protests: MutableProtest[];
  readonly lobbyingActions: MutableLobbyingAction[];
  readonly mediaInfluences: MutableMediaInfluence[];
  readonly companies: MutableCompany[];
  readonly populationCohorts: MutablePopulationCohort[];
  readonly centralBanks: MutableCentralBank[];
  readonly banks: MutableBank[];
...
function processGovernment(context: GovernmentContext): void {
  for (const government of context.governments) {
    ...
    const budget: MutableGovernmentBudget = {
...
function syncTaxPolicyFromLaws(policy: MutableTaxPolicy, activeLaws: readonly Law[], tick: number): void {
...
function syncDepositInsurance(context: GovernmentContext, government: MutableGovernment, activeLaws: readonly Law[]): void {
...
function collectGovernmentTaxes(context: GovernmentContext, government: MutableGovernment, policy: TaxPolicy, budget: MutableGovernmentBudget): void {
  const salesRevenueByCompany = new Map<string, number>();
  ...
  for (const company of context.companies) {
    ...
    const taxMinor = sanitizeMoney(
      Math.floor((revenueMinor * policy.salesTaxRate + profitBaseMinor * policy.profitTaxRate) * clamp(government.taxEfficiency, 0, 1))
    );
...
function applyGovernmentSubsidies(context: GovernmentContext, government: MutableGovernment, budget: MutableGovernmentBudget): void {
  for (const subsidy of context.subsidies) {
    ...
    const companies = context.companies.filter(
      (company) =>
        company.countryId === government.countryId &&
...
function applyGovernmentSpending(context: GovernmentContext, government: MutableGovernment, budget: MutableGovernmentBudget): void {
...
function applyBankBailouts(
  context: GovernmentContext,
  government: MutableGovernment,
  activeLaws: readonly Law[],
  budget: MutableGovernmentBudget
): void {
...
function enforceIndustryLicenses(context: GovernmentContext, government: MutableGovernment, activeLaws: readonly Law[]): void {
  for (const law of activeLaws.filter((candidate) => candidate.type === "industry_license")) {
    ...
    for (const company of context.companies) {
      ...
      const hasLicense = context.licenses.some(
        (license) =>
          license.countryId === government.countryId &&
          license.companyId === company.id &&
          license.industry === industry &&
          license.status === "active" &&
...
function updateSanctionMetrics(context: GovernmentContext, government: Government): void {
...
function updateLawAdoption(context: GovernmentContext, government: MutableGovernment): void {
...
function updateGovernmentStability(
  context: GovernmentContext,
  government: MutableGovernment,
...
function updatePublicDebt(context: GovernmentContext, government: MutableGovernment, budget: GovernmentBudget): void {
...
function getOrCreateTaxPolicy(context: GovernmentContext, countryId: string): MutableTaxPolicy {
...
function getNumberParam(law: Law | undefined, key: string, fallback: number): number {
  const value = law?.parameters[key];
  return typeof value === "number" && Number.isFinite(value) ? value : fallback;
}

function getStringParam(law: Law | undefined, key: string, fallback: string): string {
  const value = law?.parameters[key];
  return typeof value === "string" ? value : fallback;
}

function transferCompanyToGovernment(input: {
  readonly context: GovernmentContext;
  readonly company: MutableCompany;
  readonly government: Government;
  readonly budget: MutableGovernmentBudget;
  readonly amountMinor: number;
  readonly transactionType: FinancialTransaction["type"];
...
function getEnvironmentalFineMinor(context: GovernmentContext, company: Company, policy: TaxPolicy): number {
...
function subsidyMatchesCompany(context: GovernmentContext, subsidy: Subsidy, company: Company): boolean {
...
function companyMatchesIndustry(context: GovernmentContext, company: Company, industry: string): boolean {
...
interface WarContext {
  ...
  readonly shipments: MutableShipment[];
  readonly logisticsRoutes: MutableLogisticsRoute[];
  readonly infrastructureLinks: MutableInfrastructureLink[];
  readonly transportCompanies: MutableTransportCompany[];
  readonly populationCohorts: MutablePopulationCohort[];
  readonly wars: MutableWar[];
  readonly fronts: MutableFront[];
  readonly strategicCells: MutableStrategicCell[];
  readonly armies: MutableArmy[];
  readonly militaryUnits: MutableMilitaryUnit[];
  readonly militarySupplies: MutableMilitarySupply[];
  readonly occupations: MutableOccupation[];
  readonly treaties: MutableTreaty[];
  readonly sanctions: MutableSanction[];
  readonly alliances: MutableAlliance[];
  readonly blockades: MutableBlockade[];
  readonly refugeeFlows: MutableRefugeeFlow[];
  readonly warDamage: MutableWarDamage[];
  readonly militaryOrders: MutableMilitaryOrder[];
  readonly nextTick: number;
  readonly seed: string;
  readonly events: DomainEvent[];
  readonly metrics: Metric[];
...
function processWarAndGeopolitics(context: WarContext): void {
...
function processArmySupply(context: WarContext, war: MutableWar, army: MutableArmy): void {
...
function recordMilitarySupply(
  context: WarContext,
  war: MutableWar,
  army: MutableArmy,
  supplyType: "food" | "fuel" | "ammunition",
...
function processFrontMovement(context: WarContext, war: MutableWar, front: MutableFront): void {
...
function calculateArmyPower(context: WarContext, army: Army, cellIds: readonly string[]): number {
...
function captureStrategicCell(
  context: WarContext,
  war: MutableWar,
  cell: MutableStrategicCell,
  occupierCountryId: string,
  previousControllerCountryId: string
): void {
...
function createWarDamage(context: WarContext, war: MutableWar, cell: MutableStrategicCell): void {
...
function moveRefugees(context: WarContext, war: MutableWar, cell: MutableStrategicCell): void {
...
function transferCohortPopulation(cohorts: MutablePopulationCohort[], originCityId: string, destinationCityId: string, people: number): void {
...
function applyWarBlockades(context: WarContext, war: War): void {
...
function applyWarSanctions(context: WarContext, war: MutableWar): void {
...
function applyMilitaryLogisticsCompetition(context: WarContext, war: War): void {
...
function findMilitarySupplyProductId(state: WorldState, supplyType: "food" | "fuel" | "ammunition"): string {
  const suffixByType = {
  ...
  const product =
    state.products.find((candidate) => candidate.id.endsWith(suffixByType[supplyType])) ??
...
function getMilitarySupplyMaxPriceMinor(supplyType: "food" | "fuel" | "ammunition"): number {
  const priceByType = {
    food: 900,
    fuel: 1_400,
    ammunition: 2_600
...
function findNearestCity(cities: readonly MutableCity[], point: { readonly lat: number; readonly lon: number }, countryId: string): MutableCity | null {
  return (
    cities
...
function geoDistance(left: { readonly lat: number; readonly lon: number }, right: { readonly lat: number; readonly lon: number }): number {
  const lat = left.lat - right.lat;
  const lon = left.lon - right.lon;
  return Math.sqrt(lat * lat + lon * lon);
}

interface CrimeContext {
  readonly state: WorldState;
  readonly countries: readonly MutableCountry[];
  readonly cities: readonly MutableCity[];
  readonly products: readonly Product[];
  readonly companies: MutableCompany[];
  readonly populationCohorts: readonly MutablePopulationCohort[];
  readonly warehouses: readonly MutableWarehouse[];
  readonly inventoryLots: MutableInventoryLot[];
  readonly demandRecords: readonly DemandRecord[];
  readonly bankAccounts: MutableBankAccount[];
  readonly laws: readonly MutableLaw[];
  readonly taxPolicies: readonly MutableTaxPolicy[];
  readonly governments: readonly MutableGovernment[];
  readonly corruptionIndexes: readonly MutableCorruptionIndex[];
  readonly mediaInfluences: readonly MutableMediaInfluence[];
  readonly wars: readonly MutableWar[];
  readonly sanctions: readonly MutableSanction[];
  readonly blackMarkets: MutableBlackMarket[];
  readonly illegalTrades: MutableIllegalTrade[];
  readonly smugglingRoutes: MutableSmugglingRoute[];
  readonly corruptionCases: MutableCorruptionCase[];
  readonly investigations: MutableInvestigation[];
  readonly enforcementAgencies: MutableEnforcementAgency[];
  readonly fines: MutableFine[];
  readonly confiscations: MutableConfiscation[];
  readonly reputationPenalties: MutableReputationPenalty[];
  readonly illegalContracts: MutableIllegalContract[];
  readonly nextTick: number;
  readonly seed: string;
  readonly financialTransactions: FinancialTransaction[];
  readonly events: DomainEvent[];
...
function processCrimeAndBlackMarket(context: CrimeContext): void {
...
function generateBlackMarkets(context: CrimeContext): void {
  if (context.nextTick <= 1) {
    return;
  }
  ...
  for (const city of context.cities) {
    ...
    for (const product of context.products) {
      ...
      const existing = context.blackMarkets.find(
        (market) => market.countryId === country.id && market.cityId === city.id && market.productId === product.id
      );
...
function processIllegalTrades(context: CrimeContext): void {
...
function resolveCompletedIllegalTrade(
  context: CrimeContext,
  trade: MutableIllegalTrade,
  blackMarket: BlackMarket,
  seller: MutableCompany,
  product: Product
): void {
...
function resolveDetectedIllegalTrade(
  context: CrimeContext,
  trade: MutableIllegalTrade,
  blackMarket: BlackMarket,
  seller: MutableCompany,
...
function getBlackMarketTrigger(
  context: CrimeContext,
  countryId: string,
  cityId: string,
  product: Product
): BlackMarket["trigger"] | null {
...
function isProductBanned(laws: readonly Law[], countryId: string, product: Product): boolean {
...
function calculateBlackMarketDemand(
  context: CrimeContext,
  cityId: string,
  product: Product,
  trigger: BlackMarket["trigger"]
): number {
...
function calculateMarketRisk(context: CrimeContext, countryId: string, pressure: number): number {
...
function calculateIllegalTradePriceMinor(state: WorldState, blackMarket: BlackMarket, quantity: number): number {
...
function calculateIllegalTradeDetectionRisk(input: {
  readonly state: WorldState;
  readonly blackMarket: BlackMarket;
  readonly route: SmugglingRoute | null;
  ...
  const agency =
    input.state.enforcementAgencies.find((candidate) => candidate.countryId === input.blackMarket.countryId && candidate.active) ??
  ...
  const activeWar = input.state.wars.some(
    (war) => war.status === "active" && (war.attackerCountryId === input.blackMarket.countryId || war.defenderCountryId === input.blackMarket.countryId)
  );
  const sanctionPressure = input.state.sanctions.some(
    (sanction) => sanction.active && (sanction.sourceCountryId === input.blackMarket.countryId || sanction.targetCountryId === input.blackMarket.countryId)
  )
    ? 0.08
    : 0;
...
function getEnforcementAgency(context: CrimeContext, countryId: string): MutableEnforcementAgency {
  const existing =
    context.enforcementAgencies.find((agency) => agency.countryId === countryId && agency.active) ??
  ...
  const agency: MutableEnforcementAgency = {
...
function getCompanyInventoryQuantity(
  warehouses: readonly Warehouse[],
  inventoryLots: readonly InventoryLot[],
  companyId: string,
  productId: string
): number {
...
function getCityInventoryQuantity(warehouses: readonly Warehouse[], inventoryLots: readonly InventoryLot[], cityId: string, productId: string): number {
...
function consumeCompanyInventory(
  warehouses: readonly Warehouse[],
...
function payCompanyCash(company: MutableCompany, account: MutableBankAccount | null, requestedAmountMinor: number): number {
...
interface UpdateDataReliabilityInput {
  readonly existing: readonly DataReliability[];
...
function updateDataReliability(input: UpdateDataReliabilityInput): DataReliability[] {
  const byCountry = new Map<string | null, DataReliability>();
  ...
  return input.countries.map((country) => {
    ...
    const corruption =
      input.corruptionIndexes.find((candidate) => candidate.countryId === country.id)?.value ?? government?.corruptionLevel ?? 0.25;
    ...
    const manipulationRisk = clamp(
      Math.max(inheritedRisk * 0.65, corruption * 0.55 + (authoritarian ? 0.42 : 0) + (1 - country.stability) * 0.16),
      0,
      1
    );
    const score = clamp(
      (existing?.score ?? 0.72) * 0.45 +
        (1 - corruption * 0.42 - manipulationRisk * 0.28 + country.stability * 0.16 + (government?.taxEfficiency ?? 0.65) * 0.12) * 0.55,
      0.05,
      1
    );
    const grade =
      authoritarian || manipulationRisk > 0.55 ? "manipulated" : score >= 0.75 ? "high" : score >= 0.5 ? "medium" : "low";
...
interface CreateMetricChangesInput {
  readonly previousMetrics: readonly Metric[];
  readonly currentMetrics: readonly Metric[];
  readonly nextTick: number;
  readonly seed: string;
}

function createMetricChanges(input: CreateMetricChangesInput): MetricChange[] {
  ...
  return input.currentMetrics.map((metric, index) => {
    ...
    const percentChange =
      previousValue > 0 ? delta / Math.abs(previousValue) : currentValue > 0 ? 1 : 0;
...
function metricEntity(metric: Metric): { readonly entityType: string; readonly entityId: string | null } {
...
interface CreateEventCausesInput {
  readonly events: readonly DomainEvent[];
  readonly demandRecords: readonly DemandRecord[];
  readonly metrics: readonly Metric[];
  readonly nextTick: number;
...
function createEventCauses(input: CreateEventCausesInput): EventCause[] {
...
function classifyEventCause(event: DomainEvent): {
...
interface CreateEventImpactsInput {
  readonly events: readonly DomainEvent[];
...
function createEventImpacts(input: CreateEventImpactsInput): EventImpact[] {
  ...
  for (const event of input.events) {
    ...
    const relatedChange =
      input.metricChanges.find((change) => Boolean(change.entityId && event.entityIds.includes(change.entityId))) ??
...
function inferEventTargetType(event: DomainEvent): string {
  if (event.type.includes("Shipment") || event.type.includes("LogisticsRoute")) {
    return "logistics";
  }
...
function inferEventMetricName(event: DomainEvent): string {
  if (event.type === "ShortageDetectedEvent") {
...
interface CreateExplanationsInput {
  readonly state: WorldState;
  readonly countries: readonly Country[];
...
function createExplanations(input: CreateExplanationsInput): Explanation[] {
  return [
...
function createPriceExplanations(input: CreateExplanationsInput): Explanation[] {
  ...
  const products = input.products.filter((product) => {
    return (
      isPopulationNeedCategory(product.needCategory) ||
  ...
  for (const product of products) {
    ...
    const countryIds = new Set(
      input.cities.filter((city) => cityIds.has(city.id)).map((city) => city.countryId)
    );
    ...
    const averageRouteRisk = average(
      input.logisticsRoutes.map((route) =>
        route.active && !route.blockedReason ? 0.12 : 0.85
      )
    );
    const activeCountrySanctions = input.sanctions.filter(
      (sanction) => sanction.active && (!countryId || sanction.targetCountryId === countryId || sanction.sourceCountryId === countryId)
    );
    ...
    const taxPressure = countryId
      ...
      : 0;
    const productCompanies = input.companies.filter((company) =>
      input.state.retailOffers.some((offer) => offer.companyId === company.id && offer.productId === product.id)
    );
    ...
    explanations.push({
      ...
      summary: `${product.name} price pressure is ${trend}; demand ${Math.round(desiredQuantity)} units, supply ${Math.round(
        supplyQuantity
      )} units, unmet demand ${Math.round(unmetQuantity)} units.`,
...
function createShortageExplanations(input: CreateExplanationsInput): Explanation[] {
  return input.demandRecords
    ...
    .map((record, index) => {
      ...
      const shortageEvent = input.events.find(
        (event) => event.type === "ShortageDetectedEvent" && metadataString(event, "cohortId") === record.cohortId
      );
...
function createEventExplanations(input: CreateExplanationsInput): Explanation[] {
  const explanations: Explanation[] = [];
...
function createDemandChangeExplanations(input: CreateExplanationsInput): Explanation[] {
...
function createLogisticsDelayExplanations(input: CreateExplanationsInput): Explanation[] {
...
interface CreatePublicStatisticsInput {
  readonly state: WorldState;
  readonly countries: readonly Country[];
  readonly cities: readonly City[];
  readonly companies: readonly Company[];
...
function createPublicStatistics(input: CreatePublicStatisticsInput): PublicStatistic[] {
  const statistics: PublicStatistic[] = [];
  ...
  for (const country of input.countries) {
    ...
    const currentAveragePrice = weightedAverage(
      countryDemand.filter((record) => record.purchasedQuantity > 0).map((record) => ({
    ...
    for (const [category, value] of salesByCategory) {
      ...
      statistics.push(
        makePublicStatistic(input, country, reliability, `industry.profit.${category}`, companyCount > 0 ? value : 0, "minor")
      );
    }
  }
...
function makePublicStatistic(
  input: CreatePublicStatisticsInput,
  country: Country,
  reliability: DataReliability | undefined,
  metricName: string,
  value: number,
  unit: PublicStatistic["unit"]
): PublicStatistic {
...
function distortPublicStatistic(metricName: string, value: number): number {
...
interface CreateHiddenStatisticsInput {
  readonly companies: readonly Company[];
  readonly banks: readonly Bank[];
  readonly bankAccounts: readonly BankAccount[];
  readonly stocks: WorldState["stocks"];
  readonly nextTick: number;
  readonly seed: string;
}

function createHiddenStatistics(input: CreateHiddenStatisticsInput): HiddenStatistic[] {
  const publicCompanyIds = new Set(input.stocks.map((stock) => stock.companyId));
  const hidden: HiddenStatistic[] = [];
...
interface CreateForecastsInput {
  readonly state: WorldState;
  readonly products: readonly Product[];
  readonly countries: readonly Country[];
  readonly demandRecords: readonly DemandRecord[];
...
function createForecasts(input: CreateForecastsInput): Forecast[] {
...
function normalizeContributions(
  raw: readonly (readonly [string, EventCauseType, number])[]
): Explanation["causes"] {
...
function getReliabilityForCountry(reliability: readonly DataReliability[], countryId: string | null): DataReliability | null {
  if (!countryId) {
    return null;
  }
...
function getProductAveragePriceMinor(state: WorldState, demandRecords: readonly DemandRecord[], product: Product): number {
  const currentDemandPrice = weightedAverage(
    demandRecords
...
function getPreviousProductAveragePriceMinor(state: WorldState, product: Product): number {
  const previousDemand = state.demandRecords.filter(
    (record) => record.needCategory === product.needCategory && record.purchasedQuantity > 0 && record.averagePriceMinor > 0
  );
...
function getPreviousCountryAveragePriceMinor(state: WorldState, countryId: string): number {
  const cityIds = new Set(state.cities.filter((city) => city.countryId === countryId).map((city) => city.id));
  const previousDemand = state.demandRecords.filter(
    (record) => cityIds.has(record.cityId) && record.purchasedQuantity > 0 && record.averagePriceMinor > 0
  );
...
function latestStatistic(statistics: readonly PublicStatistic[], countryId: string, metricName: string): PublicStatistic | null {
  return (
    statistics
...
function metadataNumber(event: DomainEvent, key: string): number {
  const value = event.metadata[key];

  return typeof value === "number" && Number.isFinite(value) ? value : 0;
}

function metadataString(event: DomainEvent, key: string): string | null {
  const value = event.metadata[key];

  return typeof value === "string" && value.length > 0 ? value : null;
}

function totalMetricValue(metrics: readonly Metric[], name: string): number {
...
function isPopulationNeedCategory(value: Product["needCategory"]): value is NeedCategory {
  return NEED_CATEGORIES.includes(value as NeedCategory);
}
...
function createNews(input: CreateNewsInput): NewsItem[] {
  const news: NewsItem[] = [];
  const revenueMinor = input.financialTransactions.reduce(
    (total, transaction) =>
      total + transaction.entries.filter((entry) => entry.ownerType === "company").reduce((sum, entry) => sum + entry.amountMinor, 0),
    0
  );
  ...
  const lowestStability = input.metrics
    ...
    .reduce<number | null>((minimum, metric) => (minimum === null ? metric.value : Math.min(minimum, metric.value)), null);
...
function decorateNewsItem(item: NewsItem, input: CreateNewsInput): NewsItem {
  const category = item.category ?? inferNewsCategory(item);
  const template = input.newsTemplates.find(
    (candidate) => candidate.active && candidate.category === category && item.id.includes(candidate.eventType.toLocaleLowerCase().split("event")[0] ?? "")
  );
...
function inferNewsCategory(item: NewsItem): NonNullable<NewsItem["category"]> {
...
function calculateNeedQuantity(cohort: PopulationCohort, needCategory: NeedCategory): number {
...
function getAvailableQuantity(inventoryLots: readonly InventoryLot[], warehouseId: string, productId: string): number {
  return sanitizeQuantity(
    inventoryLots
...
function normalizeWorldState(state: WorldState): WorldState {
  return {
    ...state,
...
export function assertNoInvalidEconomyValues(state: WorldState): void {
