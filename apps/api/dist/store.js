...
class PrismaWorldStore {
    ...
    async loadWorld() {
        ...
        if (snapshot) {
            const snapshotState = upgradeWorldState(snapshot.payload, this.seed);
...
async function hydrateWorldFromNormalizedReadModel(prisma, snapshotState, seed) {
...
async function collectNormalizedReadModel(prisma) {
    ...
    const financialTransactions = mapRows(await readRows(prisma.financialTransaction, "financialTransactions", { include: { entries: true }, orderBy: [{ tick: "asc" }, { id: "asc" }] }), toFinancialTransaction);
    ...
    const playerCommands = mapRows(await readRows(prisma.playerCommandRecord, "playerCommands", { orderBy: [{ tickReceived: "asc" }, { id: "asc" }] }), toPlayerCommandRecord);
...
async function readRows(delegate, _source, args) {
...
function tupleSource(name, values) {
...
function toCompany(row) {
    return {
        ...
        ownerType: stringField(row, "ownerType"),
        ownerId: stringField(row, "ownerId"),
        countryId: stringField(row, "countryId"),
        name: stringField(row, "name"),
        legalStatus: stringField(row, "legalStatus"),
        cashBalanceMinor: numberField(row, "cashBalanceMinor"),
        currencyCode: stringField(row, "currencyCode"),
        reputation: numberField(row, "reputation"),
        bankruptcyStatus: stringField(row, "bankruptcyStatus")
    };
}
function toBankAccount(row) {
    return {
        ...
        ownerType: stringField(row, "ownerType"),
        ownerId: stringField(row, "ownerId"),
        accountType: stringField(row, "accountType"),
        currencyCode: stringField(row, "currencyCode"),
...
function toCreditScore(row) {
    return {
        id: stringField(row, "id"),
        borrowerType: stringField(row, "borrowerType"),
...
function toWarehouse(row) {
    return {
        ...
        name: stringField(row, "name"),
        warehouseType: stringField(row, "warehouseType"),
...
function toResourcePurchase(row) {
    return {
        id: stringField(row, "id"),
...
function toManualProductionRun(row) {
    return {
        id: stringField(row, "id"),
        tick: numberField(row, "tick"),
        playerId: stringField(row, "playerId"),
        ...
        inputConsumptions: jsonArray(row, "inputConsumptions"),
        status: stringField(row, "status")
    };
}
function toRetailPriceChange(row) {
    return {
        id: stringField(row, "id"),
        tick: numberField(row, "tick"),
        playerId: stringField(row, "playerId"),
...
function toFinancialTransaction(row) {
...
function toNewsItem(row) {
    return {
        id: stringField(row, "id"),
        tick: numberField(row, "tick"),
        category: (nullableString(row, "category") ?? undefined),
...
function toEventCause(row) {
    return {
        id: stringField(row, "id"),
        eventId: stringField(row, "eventId"),
        tick: numberField(row, "tick"),
        causeType: stringField(row, "causeType"),
...
function dateStringField(row, key) {
    const value = row[key];
